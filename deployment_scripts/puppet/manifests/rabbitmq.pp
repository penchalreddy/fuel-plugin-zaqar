notice('MODULAR: zaqar/rabbitmq.pp')
...
if $queue_provider == 'rabbitmq' {
  ...
  $amqp_port       = pick($zaqar_hash['rabbit_port'], '55671')
  ...
  $rabbitmq_bind_ip_address     = pick(get_network_role_property('zaqar/api', 'ipaddr'), 'UNSET')
  ...
  if ($enabled) {
    class { '::rabbitmq':
      ...
      default_user               => 'zaqar',
      default_pass               => $zaqar_hash['metadata']['rabbit_password'],
    ...
    class { 'zaqar::rabbitmq':
      enabled        => $enabled,
      # Do not install rabbitmq from zaqar classes
      rabbitmq_class => false,
      userid         => $zaqar_hash['metadata']['rabbit_user'],
      password       => $zaqar_hash['metadata']['rabbit_password'],
      require        => Class['::rabbitmq'],
    }
    ...
    if ($use_pacemaker) {
      class { 'pacemaker_wrappers::rabbitmq':
        ...
        debug           => $debug,
        erlang_cookie   => $erlang_cookie,
        admin_user      => $zaqar_hash['metadata']['rabbit_user'],
        admin_pass      => $zaqar_hash['metadata']['rabbit_password'],
        before          => Class['zaqar::rabbitmq'],
      }
    }
  }
